...
async function createTables() {
  ...
  // Things that come back around — a membership, rent, a birthday. Only the
  // rule is stored, never the individual occurrences: those are worked out
  // from the start date whenever they are needed.
  await sql`
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_repeats_user ON repeats(user_id)`;
...
export async function getState(userId) {
  const [jars, wallet, savings, funding, repeats, pref] = await Promise.all([
    ...
    sql`SELECT id, name, amount, cadence, step, start_date AS "startDate", note, created_at
        FROM repeats WHERE user_id = ${userId} ORDER BY created_at`,
  ...
  return {
    ...
    repeats: repeats.map(r => ({ ...r, createdAt: Number(r.created_at) })),
...
export async function replaceRepeats(userId, repeats) {
  const queries = [sql`DELETE FROM repeats WHERE user_id = ${userId}`];
  for (const r of repeats || []) {
    const cadence = ['week', 'month', 'year'].includes(r.cadence) ? r.cadence : 'month';
    queries.push(sql`
      INSERT INTO repeats (user_id, id, name, amount, cadence, step, start_date, note, created_at)
      VALUES (${userId}, ${String(r.id)}, ${String(r.name || '')},
              ${Number(r.amount) || 0}, ${cadence},
              ${Math.max(1, Math.min(52, Number(r.step) || 1))},
              ${String(r.startDate || '')}, ${String(r.note || '')},
